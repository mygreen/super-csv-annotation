 ...
 *     {@literal @CsvNumberMax("1000")}
 *     private long id;
 *     
 *     // 値は書式に沿った形式で入力します。
 *     {@literal @CsvColumn(number=2)}
 *     {@literal @CsvNumberFormat(pattern="#,##0")}
 *     {@literal @CsvNumberMax("1,000,000,000")}
 *     private Integer salary;
 *     
 *     // 比較する際には指定した値より小さいかどうかで判定します。
 *     {@literal @CsvColumn(number=3)}
 *     {@literal @CsvNumberMax(value="100.0", inclusive=false)}
 *     private Double rate;
 *     
 *     // getter/setterは省略
 * }
...
@Documented
@Repeatable(CsvNumberMax.List.class)
@CsvConstraint(value={})
public @interface CsvNumberMax {
     ...
     *   <li>printer : カラムの値に対数するフォーマッタです。{@link TextPrinter#print(Object)}でvalidatedValue, maxの値を文字列に変換します。</li>
     * </ul>
     * 
     * @return 省略した場合は、適用された{@link CellProcessor}に基づいたメッセージが出力されます。
     */
    String message() default "{com.github.mygreen.supercsv.annotation.constraint.CsvNumberMax.message}";
