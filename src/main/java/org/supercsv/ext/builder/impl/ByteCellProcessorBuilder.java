...
/**
 * byte/Byte型の{@link CellProcessor}を組み立てるクラス。
 *
 * @version 1.2
 * @author T.TSUCHIE
 *
...
public class ByteCellProcessorBuilder extends AbstractNumberCellProcessorBuilder<Byte> {
    ...
    protected CellProcessor buildInputCellProcessorWithConvertNullTo(final Class<Byte> type, final Annotation[] annos,
        ...
        if(type.isPrimitive() && csvColumnAnno.optional() && csvColumnAnno.inputDefaultValue().isEmpty()) {
            return prependConvertNullToProcessor(type, annos, cellProcessor, (byte)0);
            
        } else if(!csvColumnAnno.inputDefaultValue().isEmpty()) {
            
            Optional<Byte> value = parseValue(type, annos, csvColumnAnno.inputDefaultValue());
    ...
    public CellProcessor buildOutputCellProcessor(final Class<Byte> type, final Annotation[] annos,
        ...
        final Optional<Byte> min = getMin(converterAnno).map(s -> parseValue(type, annos, s).get());
        final Optional<Byte> max = getMax(converterAnno).map(s -> parseValue(type, annos, s).get());
    ...
    public CellProcessor buildInputCellProcessor(final Class<Byte> type, final Annotation[] annos,
        ...
        if(formatter.isPresent()) {
            cp = (cp == null ?
                    new ParseLocaleNumber<Byte>(type, formatter.get(), lenient) :
                        new ParseLocaleNumber<Byte>(type, formatter.get(), lenient, cp));
        } else {
            cp = (cp == null ?
    ...
    @Override
    public Optional<Byte> parseValue(final Class<Byte> type, final Annotation[] annos, final String strValue) {
