...
/**
 * {@link MonthDayProcessorBuilder}のテスタ
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public class MonthDayProcessorBuilderTest {
    ...
    private static final String TEST_FORMATTED_PATTERN = "M/d";
    
    private static final MonthDay TEST_VALUE_1_OBJ = new MonthDay(2, 29);
    private static final String TEST_VALUE_1_STR_NORMAL = "02-29";
    private static final String TEST_VALUE_1_STR_FORMATTED = "2/29";
    
    private static final String TEST_VALUE_1_STR_LENIENT = "01-60";
    
    @CsvBean
    private static class TestCsv {
        
        @CsvColumn(number=1)
        MonthDay col_default;
        ...
        MonthDay col_lenient;
        
        @CsvColumn(number=10)
        @CsvDateTimeFormat(message="テストメッセージ")
        MonthDay col_message;
        
        @CsvColumn(number=11)
        @CsvDateTimeFormat(message="")
        MonthDay col_message_empty;
        ...
        MonthDay col_message_variables;
    ...
    public void testBuildForReading_default() {
        ...
        
        {
            ...
            MonthDay expected = TEST_VALUE_1_OBJ;
            assertThat((Object)processor.execute(input, ANONYMOUS_CSVCONTEXT)).isEqualTo(expected);
        }
    ...
    public void testBuildForWriging_default() {
        ...
        {
            // valid input
            MonthDay input = TEST_VALUE_1_OBJ;
    ...
    public void testBuildForReading_leinent() {
        ...
        
        {
            ...
            MonthDay expected = TEST_VALUE_1_OBJ;
            assertThat((Object)processor.execute(input, ANONYMOUS_CSVCONTEXT)).isEqualTo(expected);
//            assertThat(((MonthDay)processor.execute(input, ANONYMOUS_CSVCONTEXT)).toDate()).isEqualTo(expected.toDate());
...
//            assertThat(((MonthDay)processor.execute(input, ANONYMOUS_CSVCONTEXT)).toDate()).isEqualTo(expected.toDate());
    ...
    public void testErrorMessage_format() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 2列] : 項目「col_format」の値（abc）は、日時の書式「M/d」として不正です。");                
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=abc, pattern=M/d");                
        }
    ...
    public void testErrorMessage_typeMismatch() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「col_default」の値（abc）は、月日の書式「MM-dd」として不正です。");                
        }
