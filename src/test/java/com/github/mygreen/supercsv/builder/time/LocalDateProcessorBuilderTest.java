...
/**
 * {@link LocalDateProcessorBuilder}のテスタ
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public class LocalDateProcessorBuilderTest {
    ...
    private static final String TEST_VALUE_1_STR_LENIENT = "2016-01-60";
    
    @CsvBean
    private static class TestCsv {
        
        @CsvColumn(number=1)
        LocalDate col_default;
        
        @CsvColumn(number=2)
        @CsvDateTimeFormat(pattern=TEST_FORMATTED_PATTERN)
        LocalDate col_format;
        
        @CsvColumn(number=3)
        @CsvDateTimeFormat(lenient=true)
        LocalDate col_lenient;
        
        @CsvColumn(number=10)
        @CsvDateTimeFormat(message="テストメッセージ")
        LocalDate col_message;
        
        @CsvColumn(number=11)
        @CsvDateTimeFormat(message="")
        LocalDate col_message_empty;
        ...
        LocalDate col_message_variables;
    ...
    public void testBuildForReading_default() {
        ...
        
        {
            ...
            LocalDate expected = TEST_VALUE_1_OBJ;
            assertThat((Object)processor.execute(input, ANONYMOUS_CSVCONTEXT)).isEqualTo(expected);
        }
    ...
    public void testBuildForWriging_default() {
        ...
        {
            // valid input
            LocalDate input = TEST_VALUE_1_OBJ;
    ...
    public void testErrorMessage_format() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 2列] : 項目「col_format」の値（abc）は、日時の書式「uu/M/d」として不正です。");                
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=abc, pattern=uu/M/d");                
        }
    ...
    public void testErrorMessage_typeMismatch() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「col_default」の値（abc）は、日付の書式「uuuu-MM-dd」として不正です。");                
        }
