...
/**
 * {@link LocalDateTimeProcessorBuilder}のテスタ
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public class LocalDateTimeProcessorBuilderTest {
    ...
    private static class TestCsv {
        ...
        LocalDateTime col_message_variables;
    ...
    public void testBuildForWriging_default() {
        ...
        {
            // valid input
            LocalDateTime input = TEST_VALUE_1_OBJ;
    ...
    public void testErrorMessage_format() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 2列] : 項目「col_format」の値（abc）は、日時の書式「uu/M/d H:m:s」として不正です。");                
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=abc, pattern=uu/M/d H:m:s");                
        }
    ...
    public void testErrorMessage_typeMismatch() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「col_default」の値（abc）は、日時の書式「uuuu-MM-dd HH:mm:ss」として不正です。");                
        }
