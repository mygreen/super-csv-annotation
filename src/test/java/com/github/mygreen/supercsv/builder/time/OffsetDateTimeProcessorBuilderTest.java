...
/**
 * {@link OffsetDateTimeProcessorBuilder}のテスタ
 *
 * @since 2.1
 * @author T.TSUCHIE
 *
 */
public class OffsetDateTimeProcessorBuilderTest {
    ...
    private static final String TEST_FORMATTED_PATTERN = "uu/M/d H:m:sxxx";
    
    private static final OffsetDateTime TEST_VALUE_1_OBJ = OffsetDateTime.of(2016, 2, 29, 7, 12, 1, 0, ZoneOffset.ofHours(9));
    ...
    private static final OffsetDateTime TEST_VALUE_1_OBJ_TIMEZONE = OffsetDateTime.of(2016, 2, 29, 16, 12, 1, 0, ZoneOffset.ofHours(9));
    private static final String TEST_VALUE_1_STR_TIMEZONE = "2016-02-28 22:12:01+00:00";
    
    @CsvBean
    private static class TestCsv {
        
        @CsvColumn(number=1)
        OffsetDateTime col_default;
        
        @CsvColumn(number=2)
        @CsvDateTimeFormat(pattern=TEST_FORMATTED_PATTERN)
        OffsetDateTime col_format;
        
        @CsvColumn(number=3)
        @CsvDateTimeFormat(lenient=true)
        OffsetDateTime col_lenient;
        
        @CsvColumn(number=4)
        @CsvDateTimeFormat(timezone="GMT")
        OffsetDateTime col_timezone;
        
        @CsvColumn(number=10)
        @CsvDateTimeFormat(message="テストメッセージ")
        OffsetDateTime col_message;
        
        @CsvColumn(number=11)
        @CsvDateTimeFormat(message="")
        OffsetDateTime col_message_empty;
        ...
        OffsetDateTime col_message_variables;
    ...
    public void testBuildForReading_default() {
        ...
        
        {
            ...
            OffsetDateTime expected = TEST_VALUE_1_OBJ;
            assertThat((Object)processor.execute(input, ANONYMOUS_CSVCONTEXT)).isEqualTo(expected);
        }
    ...
    public void testBuildForWriging_default() {
        ...
        {
            // valid input
            OffsetDateTime input = TEST_VALUE_1_OBJ;
    ...
    public void testErrorMessage_format() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 2列] : 項目「col_format」の値（abc）は、日時の書式「uu/M/d H:m:sxxx」として不正です。");                
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=abc, pattern=uu/M/d H:m:sxxx");                
        }
    ...
    public void testErrorMessage_typeMismatch() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「col_default」の値（abc）は、日時の書式「uuuu-MM-dd HH:mm:ssxxx」として不正です。");                
        }
