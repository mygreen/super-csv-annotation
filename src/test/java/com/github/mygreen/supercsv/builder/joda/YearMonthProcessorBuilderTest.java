...
public class YearMonthProcessorBuilderTest {
    ...
    private static final String TEST_FORMATTED_PATTERN = "yy/M";
    
    private static final YearMonth TEST_VALUE_1_OBJ = new YearMonth(2016, 2);
    private static final String TEST_VALUE_1_STR_NORMAL = "2016-02";
    private static final String TEST_VALUE_1_STR_FORMATTED = "16/2";
    
    private static final String TEST_VALUE_1_STR_LENIENT = "2016-02";
    ...
    public void testBuildForReading_default() {
        ...
        
        {
            ...
            assertThat((YearMonth)processor.execute(input, ANONYMOUS_CSVCONTEXT)).isEqualTo(expected);
        }
    ...
    public void testBuildForReading_leinent() {
        ...
        
        {
            ...
            YearMonth expected = TEST_VALUE_1_OBJ;
            assertThat((Object)processor.execute(input, ANONYMOUS_CSVCONTEXT)).isEqualTo(expected);
//            assertThat(((YearMonth)processor.execute(input, ANONYMOUS_CSVCONTEXT)).toDate()).isEqualTo(expected.toDate());
        }
...
//            assertThat(((YearMonth)processor.execute(input, ANONYMOUS_CSVCONTEXT)).toDate()).isEqualTo(expected.toDate());
    ...
    public void testErrorMessage_format() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 2列] : 項目「col_format」の値（abc）は、日時の書式「yy/M」として不正です。");                
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=abc, pattern=yy/M");                
        }
    ...
    public void testErrorMessage_typeMismatch() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「col_default」の値（abc）は、年月の書式「yyyy-MM」として不正です。");                
        }
