...
/**
 * {@link NumberMaxFactory}のテスタ。
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public class NumberMaxFactoryTest {
    
    @Rule
    public TestName name = new TestName();
    
    private NumberMaxFactory<Integer> factory;
    ...
    public void setUp() throws Exception {
        this.factory = new NumberMaxFactory<Integer>();
    ...
    private static final Integer TEST_VALUE_MAX_OBJ = 1000;
    private static final String TEST_VALUE_MAX_STR_FORMATTED = "1,000";
    ...
    private static class TestCsv {
        ...
        @CsvNumberMax(value=TEST_VALUE_MAX_STR_FORMATTED)
        private Integer col_default;
        
        @CsvColumn(number=2)
        @CsvNumberFormat(pattern=TEST_FORMATTED_PATTERN)
        @CsvNumberMax(value=TEST_VALUE_MAX_STR_FORMATTED, inclusive=false)
        private Integer col_inclusive_false;
        
        @CsvColumn(number=10)
        @CsvNumberFormat(pattern=TEST_FORMATTED_PATTERN)
        @CsvNumberMax(value=TEST_VALUE_MAX_STR_FORMATTED, message="テストメッセージ")
        private Integer col_message;
        
        @CsvColumn(number=11)
        @CsvNumberFormat(pattern=TEST_FORMATTED_PATTERN)
        @CsvNumberMax(value=TEST_VALUE_MAX_STR_FORMATTED, message="")
    ...
    public void testCreate_default() {
        ...
        CsvNumberMax anno = field.getAnnotationsByGroup(CsvNumberMax.class, groupEmpty).get(0);
        
        {
            ...
            assertThat(processor.get()).isInstanceOf(NumberMax.class);
            
            NumberMax<Integer> actual = (NumberMax<Integer>)processor.get();
            ...
            assertThat(actual.getValidationMessage()).isEqualTo("{com.github.mygreen.supercsv.annotation.constraint.CsvNumberMax.message}");
        }
    ...
    public void testCreate_attrValue_wrong() {
        ...
        try {
            ...
            .hasMessage("'%s' において、アノテーション @CsvNumberMax の属性 'value' の値（aaaa）は、java.lang.Integer (#,###)として不正です。", 
    ...
    public void testErrorMessage_default() {
        ...
        assertThat(processor).hasCellProcessor(NumberMax.class);
        
        String input = TEST_VALUE_PLUS1_STR_FORMATTED_2;
        try {
            ...
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「カラム1」の値（1,001）は、1,000以下の値でなければなりません。");
        }
    ...
    public void testErrorMessage_attrInclusive_false() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 2列] : 項目「col_inclusive_false」の値（1,000）は、1,000より小さい値でなければなりません。");
        }
    ...
    public void testErrorMessage_empty() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 11列] : 項目「col_message_empty」の値（1,001）は、1,000以下の値でなければなりません。");
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=1,001, max=1,000, inclusive=true");
        }
