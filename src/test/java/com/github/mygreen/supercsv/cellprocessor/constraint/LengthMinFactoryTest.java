...
/**
 * {@link LengthMinFactory}のテスタ
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public class LengthMinFactoryTest {
    
    @Rule
    public TestName name = new TestName();
    
    private LengthMinFactory factory;
    ...
    public void setUp() throws Exception {
        this.factory = new LengthMinFactory();
    ...
    private static class TestCsv {
        ...
        @CsvLengthMin(value=5)
        private String col_default;
        
        @CsvColumn(number=10)
        @CsvLengthMin(value=5, message="テストメッセージ")
        private String col_message;
        
        @CsvColumn(number=11)
        @CsvLengthMin(value=5, message="")
        private String col_message_empty;
        
        @CsvColumn(number=12)
        @CsvLengthMin(value=5,
                message="lineNumber={lineNumber}, rowNumber={rowNumber}, columnNumber={columnNumber}, label={label}, validatedValue={validatedValue}, length={length}, min={min}")
    ...
    private static class ErrorCsv {
        ...
        @CsvColumn(number=1, label="カラム1")
        @CsvLengthMin(value=-1)
        private String col_value_minus;
    ...
    public void testCreate_default() {
        ...
        CsvLengthMin anno = field.getAnnotationsByGroup(CsvLengthMin.class, groupEmpty).get(0);
        
        {
            ...
            assertThat(processor.get()).isInstanceOf(LengthMin.class);
            
            LengthMin actual = (LengthMin)processor.get();
            ...
            assertThat(actual.getValidationMessage()).isEqualTo("{com.github.mygreen.supercsv.annotation.constraint.CsvLengthMin.message}");
        }
     ...
     */
    @Test
    public void testCreate_attrMin_minus() {
        
        FieldAccessor field = getFieldAccessor(ErrorCsv.class, "col_value_minus", comparator);
        StringProcessorBuilder builder = (StringProcessorBuilder) builderResolver.resolve(String.class);
        TextFormatter<String> formatter = builder.getFormatter(field, config);
        ...
        try {
            ...
            assertThat(e).isInstanceOf(SuperCsvInvalidAnnotationException.class)
                .hasMessage("'%s' において、アノテーション @CsvLengthMin の属性 'value' の値（-1）は、0以上の値を設定してください。",
    ...
    public void testErrorMessage_default() {
        ...
        assertThat(processor).hasCellProcessor(LengthMin.class);
        ...
        try {
            ...
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「カラム1」の文字列長（4）は、5文字以上でなければなりません。");
        }
    ...
    public void testErrorMessage_empty() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 11列] : 項目「col_message_empty」の文字列長（4）は、5文字以上でなければなりません。");
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=abcd, length=4, min=5");
        }
