...
/**
 * {@link DateTimeMinFactory}のテスタ
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public class DateTimeMinFactoryTest {
    
    @Rule
    public TestName name = new TestName();
    
    private DateTimeMinFactory<Date> factory;
    ...
    public void setUp() throws Exception {
        this.factory = new DateTimeMinFactory<Date>();
    ...
    private static class TestCsv {
        ...
        @CsvDateTimeMin(value=TEST_VALUE_MIN_FORMATTED)
        private Date col_default;
        
        @CsvColumn(number=2)
        @CsvDateTimeFormat(pattern="yyyy/MM/dd")
        @CsvDateTimeMin(value=TEST_VALUE_MIN_FORMATTED, inclusive=false)
        private Date col_inclusive_false;
        
        @CsvColumn(number=10)
        @CsvDateTimeFormat(pattern="yyyy/MM/dd")
        @CsvDateTimeMin(value=TEST_VALUE_MIN_FORMATTED, message="テストメッセージ")
        private Date col_message;
        
        @CsvColumn(number=11)
        @CsvDateTimeFormat(pattern="yyyy/MM/dd")
        @CsvDateTimeMin(value=TEST_VALUE_MIN_FORMATTED, message="")
    ...
    public void testCreate_default() {
        ...
        CsvDateTimeMin anno = field.getAnnotationsByGroup(CsvDateTimeMin.class, groupEmpty).get(0);
        
        {
            ...
            assertThat(processor.get()).isInstanceOf(DateTimeMin.class);
            
            DateTimeMin<Date> actual = (DateTimeMin<Date>)processor.get();
            ...
            assertThat(actual.getValidationMessage()).isEqualTo("{com.github.mygreen.supercsv.annotation.constraint.CsvDateTimeMin.message}");
        }
    ...
    public void testCreate_attrValue_wrong() {
        ...
        try {
            ...
            .hasMessage("'%s' において、アノテーション @CsvDateTimeMin の属性 'value' の値（aaaa）は、java.util.Date (yyyy/MM/dd)として不正です。", 
    ...
    public void testErrorMessage_default() {
        ...
        assertThat(processor).hasCellProcessor(DateTimeMin.class);
        ...
        try {
            ...
            assertThat(messages).hasSize(1)
                    .contains("[2行, 1列] : 項目「カラム1」の値（1999/12/31）は、2000/01/01以降の値でなければなりません。");
        }
    ...
    public void testErrorMessage_attrInclusive_false() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 2列] : 項目「col_inclusive_false」の値（1999/12/31）は、2000/01/01より後の値でなければなりません。");
        }
    ...
    public void testErrorMessage_empty() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("[2行, 11列] : 項目「col_message_empty」の値（1999/12/31）は、2000/01/01以降の値でなければなりません。");
        }
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=1, rowNumber=2, columnNumber=12, label=col_message_variables, validatedValue=1999/12/31, min=2000/01/01, inclusive=true");
        }
