...
/**
 * {@link UniqueHashCodeFactory}のテスタ
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public class UniqueHashCodeFactoryTest {
    
    @Rule
    public TestName name = new TestName();
    
    private UniqueHashCodeFactory<Integer> factory;
    ...
    public void setUp() throws Exception {
        this.factory = new UniqueHashCodeFactory<Integer>();
    ...
    private static class TestCsv {
        ...
        @CsvColumn(number=10)
        @CsvNumberFormat(pattern="#,###")
        @CsvUniqueHashCode(message="テストメッセージ")
        private Integer col_message;
        
        @CsvColumn(number=11)
        @CsvNumberFormat(pattern="#,###")
        @CsvUniqueHashCode(message="")
        private Integer col_message_empty;
        
        @CsvColumn(number=12)
        @CsvNumberFormat(pattern="#,###")
        @CsvUniqueHashCode(message="lineNumber={lineNumber}, rowNumber={rowNumber}, columnNumber={columnNumber}, label={label}, validatedValue=${printer.print(validatedValue)}, duplicatedLineNumber={duplicatedLineNumber}, duplicatedRowNumber={duplicatedRowNumber}, hashCode={hashCode}")
    ...
    public void testCreate_default() {
        ...
        CsvUniqueHashCode anno = field.getAnnotationsByGroup(CsvUniqueHashCode.class, groupEmpty).get(0);
        
        {
            ...
            assertThat(processor.get()).isInstanceOf(UniqueHashCode.class);
            
            UniqueHashCode<Integer> actual = (UniqueHashCode<Integer>)processor.get();
            ...
            assertThat(actual.getValidationMessage()).isEqualTo("{com.github.mygreen.supercsv.annotation.constraint.CsvUniqueHashCode.message}");
        }
    ...
    public void testErrorMessage_default() {
        ...
        assertThat(processor).hasCellProcessor(UniqueHashCode.class);
    ...
    public void testErrorMessage_variables() {
        ...
        try {
            ...
            Integer rejectValue = (Integer) exception.getRejectedValue();

            List<String> messages = exceptionConverter.convertAndFormat((SuperCsvValidationException)e, beanMapping);
            assertThat(messages).hasSize(1)
                    .contains("lineNumber=2, rowNumber=3, columnNumber=12, label=col_message_variables, validatedValue=1,000, duplicatedLineNumber=1, duplicatedRowNumber=2, hashCode=" + rejectValue.hashCode());
        }
